...
| `FRONTEND_URL` | optional (defaults to `http://localhost:5173`) | `apps/api/src/index.ts`, `apps/api/src/services/slackNotifier.ts` | CORS origin allowlist and base URL for deep-links in Slack ticket notifications. Set to the web app's public URL in prod. |
