...
| `FRONTEND_URL` | optional (defaults to `http://localhost:5173`) | `apps/api/src/index.ts`, `apps/api/src/services/slackNotifier.ts` | CORS `origin` allowlist. Also used to build deep-link URLs in Slack ticket notifications. Set to the web app's public URL in prod. |
